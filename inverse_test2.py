...
def generate_super_shape(
    ...
    # --- layers: HARD toggle, true off-state below 0.5 (no center dot) ---
    if layer_weight > 0.5:
...
def fit_shape(target, maxiter=150, popsize=20, seed=0, workers=-1, verbose=True):
    ...
    bounds = ([(0.0, 1.0)]*7      # 4 weights + hollow + split + invert
...
def make_circle_in_octagon(resolution=209,
    ...
    oct_radius_frac  : octagon circumradius (center-to-vertex) as image fraction.
    circ_radius_frac : radius of the inner circular hole as image fraction.
    Octagon is flat-top oriented.
    """
    yy, xx = np.mgrid[0:resolution, 0:resolution]
    x = xx / (resolution - 1) - cx_frac
    y = yy / (resolution - 1) - cy_frac
